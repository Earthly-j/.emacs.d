...
(defun my/org-make-ascii-title ()
  ...
  (if (org-at-heading-p)
      (let* ((level (org-current-level))
	      ...
	      (command
              (cond
               ;; Pipe the text using echo so toilet treats it as one string
               ((= level 1) (format "echo %s | toilet -d %s -f cybermedium -F gay"
                                    (shell-quote-argument heading-text)
                                    (shell-quote-argument font-dir)))
               ((= level 2) (format "echo %s | toilet -d %s -f slant -F metal"
                                    (shell-quote-argument heading-text)
                                    (shell-quote-argument font-dir)))
               ((= level 3) (format "echo %s | toilet -d %s -f ascii9 -F metal"
                                    (shell-quote-argument heading-text)
                                    (shell-quote-argument font-dir)))
               (t (format "echo %s | toilet -d %s -f smblock -F gay"
                          (shell-quote-argument heading-text)
                          (shell-quote-argument font-dir)))))
					; 3 parens: closes format, cond, command
 ...
 * hello 
#+BEGIN_EXAMPLE
[1. ORG MODE] Reads cursor context
       |
       | (Level & Text)
       v
[ Shell Execution ] echo 'Text' | toilet -f font
       |
       | (Raw ANSI codes)
       v
[2. ANSI-COLOR] Translates codes to Emacs colors
       |
       | (Colored Strings)
       v
[3. SUBR-X] Checks if string is empty/whitespace
       |
       | (Valid Art?)
       v
   +---+---+
